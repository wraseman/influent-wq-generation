...
for (nsim in nsim.array) {  
  for (innov in innov.array) {
    for (stand in stand.array) {
      str_c("number of simulations: ", nsim, ", ", 
...
# # visualize_statistics <- function(nsims, innov=TRUE, data.type, standardize=TRUE) {
# 
# # inputs
...
# 
# file.boxplt <- str_c("./figures/obs-sim_boxplt_", model.type, ".tiff", sep = "")  # filename for boxplts
# 
...
# 
# file.tocstats <- str_c("./figures/obs-sim_boxplt_", model.type, ".tiff", sep = "")  # filename for boxplts
# 
...
#   
#   print("Mine data no longer supported in analysis.")
#   
