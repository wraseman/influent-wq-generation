...
# define function that runs k-NN simulations
main <- function() {
  
  # clear environment
  rm(list=ls())
  
  print("loading simulate_kNN function")
  
  # define simulate_kNN function 
  # note: this is the same as the function defined in 04_simulate_kNN.R but the file paths are 
  #   changed to match the remote workflow
  simulate_kNN <- function(nsims=100, innov=TRUE, threshold, 
    ...
    for (i in 1:nvars) {
      model.type <- str_c("kNN", str_c("nsims", "-", nsims),
                          ...
                          sep="_")
      write.path <- str_c("./data/", 
      # write.path <- str_c("./response/comp_experiment/remote/data/", 
                          model.type, 
                          "_", 
                          data.type,
                          "_",
  ...
  print("simulate_kNN() successfully loaded")
  
  # command line arguments from bash script
  args <- commandArgs(trailingOnly = TRUE)
  
  if(length(args) != 3) { stop("Exactly three arguments are needed: number of simulations, 
